...
```{r, message=FALSE, warning=FALSE}
NEON_MAGs_sankey <- read_csv("data/NEON/GOLD_Study_ID_Gs0161344_NEON_2024_4_21.csv") %>% 
...
```{r}
NEON_MAGs_ind_Verruco <- NEON_MAGs_sankey %>% 
  filter(`Assembly Type` == "Individual") %>% 
  filter(Phylum == "Verrucomicrobiota")

NEON_MAGs_co_Verruco <- NEON_MAGs_sankey %>% 
  filter(`Assembly Type` == "Combined") %>% 
  filter(Phylum == "Verrucomicrobiota")
```
...
```{r}
knitr::include_url("images/all_combined.html")
```
#### All Individual
...
write_tsv(sankey_data_allTaxa, "data/NEON/MAGs_pavian_ind")
```

```{r}
knitr::include_url("images/all_individual.html")
```
